...
void SDOTimeoutAlarm(CO_Data* d, UNS32 id)
{
	...
	sendSDOabort(d, d->transfers[id].whoami, d->transfers[id].CliServNbr,
			d->transfers[id].index, d->transfers[id].subIndex, SDOABT_TIMED_OUT);   //SDO超时发送终止报文
		...
		UNS8 subIndex, UNS32 abortCode)
{
	...
	err = sendSDOabort(d, whoami, CliServNbr, index, subIndex, abortCode);  //传输失败终止报文
	if (err) {
		...
		UNS8 subIndex, UNS32 count, UNS8 dataType, void *data, SDOCallback_t Callback, UNS8 endianize, UNS8 useBlockMode)
{
	...
	/* First let's find the corresponding SDO client in our OD  */
	CliNbr = GetSDOClientFromNodeId( d, nodeId);    //通过服务器节点ID查找客户端号
	if(CliNbr >= 0xFE)  //客户端不能大于等于254个
		return CliNbr;
	/* Verify that there is no SDO communication yet. */
	err = getSDOlineOnUse(d, CliNbr, SDO_CLIENT, &line);    //通过客户端号获取SDO传输通道，检查该客户端是否正在传输过程中，如果在直接退出
	if (!err) {
		MSG_ERR(0x1AC4, "SDO error : Communication yet established. with node : ", nodeId);
		return 0xFF;
	}
	/* Taking the line ... */
	err = getSDOfreeLine( d, SDO_CLIENT, &line );   //获取空闲的SDO传输通道
	if (err) {
    ...
    if(useBlockMode) {  //块模式
	    initSDOline(d, line, CliNbr, index, subIndex, SDO_BLOCK_DOWNLOAD_IN_PROGRESS);  //初始化SDO块传输通道
	    d->transfers[line].objsize = count;     //获取块模式的传输数据大小
    }
    else    //非块模式
	    initSDOline(d, line, CliNbr, index, subIndex, SDO_DOWNLOAD_IN_PROGRESS);    //初始化SDO块传输通道
	d->transfers[line].count = count;   //字节数
	d->transfers[line].dataType = dataType;     //数据类型
#ifdef SDO_DYNAMIC_BUFFER_ALLOCATION    //支持内存动态分配
	{
		UNS8* lineData = d->transfers[line].data;   //缓冲区指针
		if (count > SDO_MAX_LENGTH_TRANSFER)    //如果字节数大于缓冲区长度
		{
			d->transfers[line].dynamicData = (UNS8*) malloc(count); //动态分配内存
...
#  endif
    ...
    if(useBlockMode) {  //块下载
	    buf[0] = (6 << 5) | (1 << 1 );   /* CCS = 6 , CC = 0 , S = 1 , CS = 0 */    //块下载：110
 	    for (i = 0 ; i < 4 ; i++)   //字节数
		    buf[i+4] = (UNS8)((count >> (i<<3))); /* i*8 */
    }
    else {  //域下载
	    /* Send the SDO to the server. Initiate download, cs=1. */
	    if (count <= 4) { /* Expedited transfer */  //字节数小于4字节，加速下载
		    buf[0] = (UNS8)((1 << 5) | ((4 - count) << 2) | 3); //下载：001+无意义字节数+加速下载+指明数据长度
		    for (i = 4 ; i < 8 ; i++)   //拷贝数据
			    buf[i] = d->transfers[line].data[i - 4];
		    d->transfers[line].offset = count;
	    }
	    else { /** Normal transfer */   //字节数大于4字节，正常下载
		    buf[0] = (1 << 5) | 1;  //下载：001+数据字节为字节计数器
		    for (i = 0 ; i < 4 ; i++)   //数据字节数
			    buf[i+4] = (UNS8)((count >> (i<<3))); /* i*8 */
	    }
    }
	buf[1] = (UNS8)(index & 0xFF);        /* LSB */ //索引
	buf[2] = (UNS8)((index >> 8) & 0xFF); /* MSB */ 
	buf[3] = subIndex;  //子索引

	d->transfers[line].Callback = Callback;

	err = sendSDO(d, SDO_CLIENT, CliNbr, buf);      //SDO_CLIENT：客户端发送   CliNbr：客户端号（含Node-ID）   buf：数据段0-7
	if (err) {
		...
		UNS32 * abortCode)
{
	...
	* abortCode = 0;
	...
	/* First let's find the corresponding SDO client in our OD  */
	CliNbr = GetSDOClientFromNodeId(d, nodeId); //获取客户端号
	if(CliNbr >= 0xFE) {
        *size = 0;
		return SDO_ABORTED_INTERNAL;
    }
	...
	err = getSDOlineOnUse(d, CliNbr, SDO_CLIENT, &line);    //通过客户端/服务器号获取SDO传输通道号
	if (err) {
		MSG_ERR(0x1AF0, "SDO error : No line found for communication with node : ", nodeId);
        *size = 0;
        resetSDOline(d, line);   ///MLH新增一行
        return SDO_ABORTED_INTERNAL;
	}
